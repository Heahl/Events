// __tests__/auth.routes.bun.test.js
...
const ensureConnection = async () => {
    ...
    // Warte kurz, damit die Verbindung definitiv bereit ist
...
describe('Auth Routen - Registrierung', () => {
    ...
    afterEach(async () => {
        try {
            await ensureConnection(); // Stelle sicher, dass die Verbindung besteht, bevor wir aufräumen
            await User.deleteMany({});
            console.log("Benutzer erfolgreich gelöscht.");
        } catch (error) {
            console.error("Fehler beim Löschen der Benutzer:", error);
            // Optional: Erneuter Versuch
            try {
    ...
    afterAll(async () => {
        // Optional: Trenne die Verbindung am Ende aller Tests in dieser Datei
        // Achtung: Dies könnte andere Tests beeinflussen, wenn sie dieselbe Verbindung nutzen
    ...
    test('should register a new provider and return 201', async () => {
        await ensureConnection(); // Stelle sicher, dass die Verbindung besteht
