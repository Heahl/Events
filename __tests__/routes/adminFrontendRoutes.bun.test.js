// __tests__/admin.frontend.routes.bun.test.js
...
describe('Admin Frontend Routes - /admin/*', () => {
    ...
    let testUserPassword = 'SicheresPasswort!'; // An das neue PW_REGEX angepasst

    // Hilfsfunktion: Login durchführen und Session-Cookie zurückgeben
    ...
    beforeAll(async () => {
        ...
        const user = await User.create({
            email: testUserEmail,
            password: testUserPassword // <-- Klartext-Passwort, das Model kümmert sich um das Hashing
        });
    ...
    describe('GET /admin/dashboard', () => {
        ...
        test('should render dashboard with list of own events if user is authenticated', async () => {
            ...
            expect(response.text).toContain('2025-03-20');
            // Annahme: Template zeigt die Anzahl der Anmeldungen an
            expect(response.text).toContain('2 Anmeldungen');
    ...
    describe('GET /admin/event/create', () => {
        ...
        test('should render the event creation form if user is authenticated', async () => {
            ...
            expect(response.text).toContain('action="/admin/event"');
        });
    });
    ...
    describe('GET /admin/event/:id/participants', () => {
        ...
        test('should render list of participants for an event created by the authenticated user', async () => {
            ...
            const response = await request(app)
                .get(`/admin/event/${eventIdForThisTest}/participants`) // Verwende die lokale ID
                .set('Cookie', sessionCookie);
        ...
        test('should return 404 if user tries to access participants of an event from a different provider', async () => {
            ...
            const otherUser = await User.create({
                ...
                password: 'SicheresPasswort!' // <-- Klartext-Passwort für das andere Model
            });
            ...
            const sessionCookie = await loginAndGetSessionCookie(); // Vom ersten (eigenen) User
            ...
            // Passe die Erwartung an: Status 404 UND JSON-Body mit Fehlermeldung
